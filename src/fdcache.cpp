...
//encrytion files
#include <openssl/evp.h> //needed for openssl compatibility
#include <fcntl.h>
#include <fstream>
#include "rc4_enc.c"
#include "rc4_skey.c"
...
int encrypt(int fd)
{
        ...
        unsigned char* buf;
        buf=(unsigned char*) calloc(filelen,sizeof(char));
        t=pread(fd, buf,filelen,0);
        string file_str(reinterpret_cast<char*>(buf),filelen);
        

        RC4_KEY a;
        //string op_key;
        //EVP_BytesToKey(EVP_rc4(),EVP_sha256(),NULL,(unsigned char*)pwd.c_str(), strlen(pwd.c_str()),1,op_key,NULL)
        int len=file_str.size();
        unsigned char *buffer=(unsigned char*) malloc(len+1);
        memset(buffer,0,len+1);
        
        //encryption
        RC4_set_key(&a, pwd.length(), (const unsigned char*)pwd.c_str());
        RC4(&a, len, (const unsigned char*)file_str.c_str(),buffer);
        
        //write
        string r((char*)buffer,len);
        const char *obuf=r.c_str();
        t=pwrite(fd,obuf,r.length(),0);
        t=ftruncate(fd,r.length());
        return 0;
}
